...
function HistoryPage({gatewayUrl,t,language,snapshot,cellStats,chartSettings,setChartSettings}:{gatewayUrl:string;t:ReturnType<typeof translator>;language:Language;snapshot:GatewaySnapshot|null;cellStats:ReturnType<typeof calculateCellStats>;chartSettings:ChartDisplaySettings;setChartSettings:(settings:ChartDisplaySettings)=>void}) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(false);
    fetchGatewayHistory(gatewayUrl, to - duration, to)
  ...
  return <div className={`page-content history-page ${dragSelection?"selecting-time":""}`} onWheel={handleHistoryWheel} onContextMenu={handleChartContextMenu} onPointerDown={handleZoomPointerDown} onPointerMove={handleZoomPointerMove} onPointerUp={handleZoomPointerEnd} onPointerCancel={handleZoomPointerEnd} onDoubleClick={handleChartDoubleClick}>
    ...
    {points.length > 0 && <>
      ...
      <CompositeHistoryChart points={points} connectionEvents={history?.connectionEvents ?? []} socEvents={socBoundaryEvents} selectedSeries={selectedSeries} period={period} setPeriod={setPeriod} language={language} t={t} addMetric={addMetric} removeMetric={removeMetric} viewport={viewport} thresholds={thresholds} chartSettings={chartSettings} packVoltageRange={packVoltageRange} onHide={()=>hideHistorySection("compositeChart")}/>
      </>}
      {chartSettings.historySections.cellVoltageChart&&<CellVoltageHistoryChart points={points} socEvents={history?.socEvents ?? []} connectionEvents={history?.connectionEvents ?? []} supportSeries={series} setSeriesColor={setSeriesColor} setSignedColor={setSignedColor} period={period} setPeriod={setPeriod} language={language} t={t} viewport={viewport} thresholds={thresholds.filter((threshold)=>threshold.metric==="cellVoltageV")} bmsReferences={bmsThresholdReferences("cellVoltageV",snapshot?.protectionSettings,t)} chemistry={snapshot?.chemistry} protectionSettings={snapshot?.protectionSettings} chartSettings={chartSettings} setChartSettings={setChartSettings} onHide={()=>hideHistorySection("cellVoltageChart")}/>}
      ...
      {chartSettings.historySections.cellResistanceChart&&<CellResistanceHistoryChart points={points} connectionEvents={history?.connectionEvents ?? []} period={period} setPeriod={setPeriod} language={language} t={t} viewport={viewport} thresholds={thresholds} chartSettings={chartSettings} setChartSettings={setChartSettings} onHide={()=>hideHistorySection("cellResistanceChart")}/>}
      {visibleIndividualSeries.length>0&&<IndividualHistoryCharts points={points} connectionEvents={history?.connectionEvents ?? []} socEvents={socBoundaryEvents} series={visibleIndividualSeries} period={period} setPeriod={setPeriod} language={language} t={t} viewport={viewport} thresholds={thresholds} protectionSettings={snapshot?.protectionSettings} packVoltageRange={packVoltageRange} chartSettings={chartSettings} setChartSettings={setChartSettings} setSeriesColor={setSeriesColor} setSignedColor={setSignedColor}/>} 
